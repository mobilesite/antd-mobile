...
describe('SegmentedControl', () => {
  ...
  it('small size does not have closeDom', () => {
    ...
    expect(wrapper.find('.ej-tag-close')).toHaveLength(0);
  });
  ...
  it('onChange then selected', () => {
    ...
    wrapper.find('.ej-tag').simulate('click');
  ...
  it('onClose and removed', () => {
    ...
    expect(wrapper.find('.ej-tag-close')).toHaveLength(1);
    expect(wrapper.state('closed')).toEqual(false);
    wrapper.find('.ej-tag-close').simulate('click');
    expect(onClose).toHaveBeenCalled();
    expect(wrapper.state('closed')).toEqual(true);
    expect(afterClose).toHaveBeenCalled();
    expect(wrapper.find('.ej-tag')).toHaveLength(0);
  });
});
